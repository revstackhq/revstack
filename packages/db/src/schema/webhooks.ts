...
export const webhookEndpoints = revstack.table("webhook_endpoints", {
  id: text("id").$defaultFn(() => generateId("whe")).primaryKey(),
...
export const webhookDeliveries = revstack.table("webhook_deliveries", {
  id: text("id").$defaultFn(() => generateId("whd")).primaryKey(),
  endpointId: text("endpoint_id").references(() => webhookEndpoints.id, { onDelete: "cascade" }).notNull(),
